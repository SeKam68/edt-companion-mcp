...
как есть и сравнивает с информационной базой. Состав процедур при этом менять
нельзя: обработчики привязаны по имени процедуры в раскладке, и переименование
оторвало бы их молча, поэтому такие правки отклоняются. Раскладка формы
остаётся только для конфигуратора.
