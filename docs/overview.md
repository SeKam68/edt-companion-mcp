...
**HTTP MCP-сервер внутри 1C:EDT.** OSGi-bundle, JDK-builtin `HttpServer` на `127.0.0.1:6868`, JSON-RPC 2.0 + MCP-protocol. 45 инструментов: метаданные, BSL-навигация, рефакторинг-search, валидация, быстрые исправления проверок, редактирование форм/реквизитов/прав/СКД/XDTO, headless обновление ИБ, запуск и **отладка** yaxunit-тестов, профилирование, офлайн-описание проверок EDT, чтение журнала регистрации, PNG-скриншот форм. Опциональный PII-фильтр результатов.
...
См. [README.md](../README.md) → разделы «Установка» и «Подключение AI-агента». Подключается в `.mcp.json` проекта как `http://127.0.0.1:6868/mcp`. Health: `curl http://127.0.0.1:6868/health` → `{"status":"ok","tools":45}`.
