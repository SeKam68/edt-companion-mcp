...
**HTTP MCP-сервер внутри 1C:EDT.** OSGi-bundle, JDK-builtin `HttpServer` на `127.0.0.1:6868`, JSON-RPC 2.0 + MCP-protocol. 46 инструментов: метаданные, BSL-навигация, рефакторинг-search, валидация, быстрые исправления проверок, редактирование форм/реквизитов/прав/СКД/XDTO, headless обновление ИБ с наблюдением за длинной работой через `job`, запуск и **отладка** yaxunit-тестов, профилирование, офлайн-описание проверок EDT, чтение журнала регистрации, PNG-скриншот форм. Опциональный PII-фильтр результатов.
...
- **Конфигурации на обычных формах**: EDT неуправляемую форму в модель не поднимает — и раскладка, и модуль лежат в бинарном `Form.oform`. Модули читаются, ищутся и правятся (текст извлекается из контейнера и записывается обратно, с запретом менять состав процедур — обработчики привязаны по имени в раскладке), а инструменты, которые такие формы не покрывают, говорят об этом блоком `coverage` вместо молчаливого нуля. На конфигурации эпохи 8.2 это половина кода. Цикл работы — [ordinary-forms.md](ordinary-forms.md).
...
См. [README.md](../README.md) → разделы «Установка» и «Подключение AI-агента». Подключается в `.mcp.json` проекта как `http://127.0.0.1:6868/mcp` — запись делает кнопка на странице настроек плагина, вручную порт переносить не нужно. Health: `curl http://127.0.0.1:6868/health` → `{"status":"ok","tools":46,"workspace":"…","projects":[…]}` — поля `workspace` / `projects` называют, какой именно EDT отвечает на порту.
