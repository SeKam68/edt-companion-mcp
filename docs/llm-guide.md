...
`unspecified` означает ровно «не классифицировано», а не «неизвестная ошибка» — такие места размечаются по мере работы.
