...
Ограничения: один проект на пакет (переключать `projectName` внутри нельзя), вложенные пакеты не поддержаны, `dryRun` внутри элемента игнорируется. Большинство объектов имеют русские имена и регистр важен.
