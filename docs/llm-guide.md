...
- **Модуль можно править.** `write_module_source` по тому же пути перезаписывает текст
  ...
  **Состав процедур менять нельзя:** обработчики привязаны по имени процедуры в раскладке,
  поэтому переименование, добавление и удаление отклоняются. После правки — `sync_database`.
  Раскладка формы остаётся только для конфигуратора.
