...
- **Неизвестный параметр не отбрасывается молча.** Схемы открытые (лишнее поле не приводит к отказу), но параметр не из схемы называется в `warnings`: опечатка в имени фильтра иначе давала успешный ответ с НЕотфильтрованным списком.
